...
export class HomeComponent implements OnInit {
  ...
  constructor(private localService: LocalService, private sanitizer: DomSanitizer) {}
  ...
  ngOnInit(): void {
    this.localService.listar().subscribe({
      next: (data) => {
        ...
        console.log(this.locaiss)
        const rawImagem = this.locaiss[0].imagem;
  ...
  sanitizeImagem(url: string): SafeUrl {
  try {
    const parsed = JSON.parse(url);
    const finalUrl = Array.isArray(parsed) ? parsed[0] : parsed;
